...
async def setup_commands(application):
    ...
    commands = [
        ...
        BotCommand("gco", "Checkout a branch in the git repository of the current project"),
        ...
        BotCommand("gfetch", "Fetch updates from the git repository of the current project"),
        BotCommand("kill", "Kill the current Claude session"),
        BotCommand("checklogin", "Check if the bot is logged in to Claude"),
    ]
    await application.bot.set_my_commands(commands)

async def send_message(update: Update, context: ContextTypes.DEFAULT_TYPE, message: str, **kwargs):
    ...
    if not update.message:
        if not update.effective_chat:
            return
        return await context.bot.send_message(chat_id=update.effective_chat.id, text=message, **kwargs)
...
async def check_user(context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if context._user_id not in allowed_user_ids:
        try:
            await context.bot.send_message(chat_id=allowed_user_ids[0], text=f"Unauthorized access attempt by user ID: {context._user_id}")
...
def authenticated(func):
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        if not await check_user(context):
            await send_message(update, context, 'Unauthorized access. This incident has been reported.')
            return
...
async def greet_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await send_message(update, context, 'Hello! I am ClaudeBot, your assistant for managing Claude sessions and projects. Use /select to choose a project to work on.')
...
async def check_login(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_logged_in:
        ...
        await send_message(update, context, 'You are not logged in to Claude. Please review your credentials.')

@authenticated
async def pick_project(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    global current_project
    if not context.args:
        projects = [d for d in os.listdir(settings.projects_dir) if os.path.isdir(os.path.join(settings.projects_dir, d)) and not d.startswith('.') and not d.startswith('_')]
        if projects:
            keyboard = [
                [InlineKeyboardButton(project, callback_data=f"selectproject_{project}")] for project in projects
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await send_message(update, context, "Pick a project:", reply_markup=reply_markup)
        else:
            await send_message(update, context, 'No projects found. Clone a new project using /gclone command')
        return
    if not context.args:
        await send_message(update, context, 'Please specify a project name. Usage: /select <project_name>')
        return
    current_project = context.args[0]
    ret_code, output = await run_command("git rev-parse --abbrev-ref HEAD", cwd=os.path.join(settings.projects_dir, current_project))
    current_branch = output.strip() if ret_code == 0 else "unknown branch"
    await send_message(update, context, f'Session started successfully with project:\n*{current_project}* on branch *{current_branch}*', parse_mode='Markdown')

@authenticated
async def get_current_project(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if current_project:
        ret_code, output = await run_command("git rev-parse --abbrev-ref HEAD", cwd=os.path.join(settings.projects_dir, current_project))
        current_branch = output.strip() if ret_code == 0 else "unknown branch"
        await send_message(update, context, f'*{current_project}* on branch *{current_branch}*', parse_mode='Markdown')
    else:
        await send_message(update, context, 'No project selected. Please select a project using /select.')
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if claude_session:
        await send_message(update, context, 'A Claude session is already processing a message. Please wait for it to finish before sending another message.')
        return
    if update.message and update.message.text:
        ...
        if plan_mode:
            message = message[1:]
        ret, resp = await claude_session.send(message, resume_session=resume_session, plan_mode=plan_mode)
        claude_sessions.pop(current_project, None)
        if ret != 0:
            print(f"Claude process exited with code {ret}")
        await send_message(update, context, resp.strip() or "No response received from Claude.", parse_mode='Markdown')
    else:
...
async def select_project(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if option.startswith("selectproject_"):
        ...
        await query.edit_message_text(text=f'Starting Claude with project: {project_name}')
...
async def git_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not os.path.exists(project_path):
        await send_message(update, context, f'Project directory not found: {current_project}')
        return
    
    ret_code, output = await run_command("git status", cwd=project_path)
    
    if ret_code != 0:
        await send_message(update, context, f'Git status failed with code {ret_code}:\n```\n{output}\n```', parse_mode='Markdown')
    else:
        await send_message(update, context, f'```\n{output}\n```', parse_mode='Markdown')
...
async def git_diff(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if ret_code != 0:
        await send_message(update, context, f'Git diff failed with code {ret_code}:\n```\n{output}\n```', parse_mode='Markdown')
    else:
        if output.strip():
            await send_message(update, context, f'```diff\n{output}\n```', parse_mode='Markdown')
        else:
...
async def git_reset(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if ret_code != 0:
        await send_message(update, context, f'Git reset failed with code {ret_code}:\n{output}')
    else:
...
async def git_clone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not repo_url:
        await send_message(update, context, 'Please specify a repository URL. Usage: /gclone <repo_url>')
        return
    
    if not repo_url.startswith("https://") and not repo_url.startswith("git@"):
        repo_url = f"git@github.com:{repo_url}"
    
    ret_code, output = await run_command(f"git clone {repo_url}", cwd=settings.projects_dir)
    
    if ret_code != 0:
        await send_message(update, context, f'Git clone failed with code {ret_code}:\n{output}')
    else:
...
async def git_push(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not branch:
        ...
        keyboard = [
            [InlineKeyboardButton(branch, callback_data=f"gpush_{branch}")] for branch in branches
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await send_message(update, context, "Select branch to push:", reply_markup=reply_markup)
        return
    ...
    ret_commit, output_commit = await run_command('git commit -m "Update from ClaudeBot"', cwd=project_path)
    if ret_commit != 0:
        print(f"Git commit failed with code {ret_commit}:\n{output_commit}")
    ret_code, output = await run_command(f"git push -u origin {branch}", cwd=project_path)
    
    if ret_code != 0:
        await send_message(update, context, f'Git push failed with code {ret_code}:\n{output}')
    else:
...
async def git_fetch(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if ret_code != 0:
        await send_message(update, context, f'Git fetch failed with code {ret_code}:\n{output}')
    else:
...
async def git_checkout(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not branch:
        ...
        keyboard = [
            [InlineKeyboardButton(branch, callback_data=f"gco_{branch}")] for branch in branches
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await send_message(update, context, "Select branch to checkout:", reply_markup=reply_markup)
        return
...
@authenticated
async def select_branch_for_checkout(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    if not query:
        return
    await query.answer()
...
app = ApplicationBuilder().token(settings.TELEGRAM_BOT_TOKEN).post_init(setup_commands).build()
...
app.add_handler(CallbackQueryHandler(select_branch_for_checkout, pattern="^(gco_|gpush_)"))
